...
export const setLogginData = (user) => {
	return async (dispatch) => {
		...
		const responseData = async () => {
			const response = await fetch(`https://weekflow-8020a-default-rtdb.firebaseio.com/users/${user}.json`);
			if (!response.ok) {
		...
		try {
			...
			if (calendar) {
				dispatch(weekActions.setCalendar({ weeks: calendar.week, currentWeek: calendar.currentWeek }));
			} else {
...
export const exportAccountData = (userUid, accountData, week, currentWeek) => {
	return async (dispatch) => {
		const responseData = async () => {
			const response = await fetch(`https://weekflow-8020a-default-rtdb.firebaseio.com/users/${userUid}.json`, {
				method: 'PUT',
				body: JSON.stringify({
					account: {
